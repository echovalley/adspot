...
function belong_to_domain($sub_domain, $main_domain) {
  if ($sub_domain == $main_domain) return true;
  $regex = "/\.$main_domain$/";
...
function parseLinkAddr($url) {
  ...
  if (preg_match('/^http:\/\/(?:e\.)?weibo\.(?:com|cn)\/(?:n\/)?(\w+)/', $url, $m)) {
    ...
    $cssobj['params'] = array('http://m.douban.com/' . $m[1] . '/subject/' . $m[2] . '/', $m[1]);
    $cssobj['css'] = 'douban';
  } elseif (preg_match('/^http:\/\/item\.taobao\.com\/item\.htm\?(.*)$/', $url, $m)) {
    if (preg_match('/(?:^|&)id=(\d+)/', $m[1], $n )) {
      $cssobj['func'] = 'parse_taobao';
      $cssobj['params'] = array('http://a.m.taobao.com/i' . $n[1] . '.htm');
      $cssobj['css'] = 'taobao';
    }
    ...
    if (preg_match('/(?:^|&)mallstItemId=(\d+)/', $m[1], $n )) {
      ...
      $cssobj['params'] = array('http://a.m.tmall.com/i' . $n[1] . '.htm');
      $cssobj['css'] = 'tmall';
    }
  } elseif (preg_match('/^http:\/\/detail\.tmall\.com\/item\.htm\?id=(\d+)/', $url, $m)) {
    $cssobj['func'] = 'parse_tmall';
    $cssobj['params'] = array('http://a.m.tmall.com/i' . $m[1] . '.htm');
    $cssobj['css'] = 'tmall';
  } elseif (preg_match('/^http:\/\/www\.dianping\.com\/shop\/(\d+)/', $url, $m)) {
    $cssobj['func'] = 'parse_dianping';
    $cssobj['params'] = array('http://m.dianping.com/shop.aspx?pid=' . $m[1]);
    $cssobj['css'] = 'dianping';
    ...
    $cssobj['params'] = array($url, $m[2]);
    $cssobj['css'] = 'video';
    $cssobj['player'] = 'tudou';
    $cssobj['vid'] = $m[2];
  } elseif (preg_match('/^https?:\/\/soundcloud\.com\/([\w-]+)\/([\w-]+)/', $url, $m)) {
    $cssobj['func'] = 'parse_soundcloud';
    $cssobj['params'] = array($url);
    $cssobj['css'] = 'soundcloud';
    $cssobj['player'] = 'soundcloud';
  } else {
...
function create_thumbnail_100($imgurl, $imgid) {
  $dest = get_thumbnail_dir() . $imgid . '_100.jpg';
  create_thumbnail($imgurl, $dest, 100, 100, true);
}
...
function create_thumbnail($imgurl, $dest, $width, $height, $crop = false) {
  if (empty($imgurl) || empty($dest)) return;
